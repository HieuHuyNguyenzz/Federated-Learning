...
from utils import load_data, load_config, get_parameters, data_partition, to_dataloader, compute_entropy
from models.get_model import get_model
from .FedDyn import FedDyn
from .FedDynClient import FedDynClient
...
class Simulation:
    ...
    def _prepare_data(self):
        ...
        client_dict, labels_dist = data_partition(trainset, self.config['flwr']['num_clients'], self.config['dataset']['alpha'])
        self.entropies = [compute_entropy(labels_dist[i]) for i in range(self.config['flwr']['num_clients'])]
    ...
    def _create_client_fn(self):
        def client_fn(context: Context) -> FedDynClient:
            ...
            return FedDynClient(cid=cid, net=model, trainloader=trainloader).to_client()
